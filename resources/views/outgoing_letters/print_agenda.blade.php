...
<head>
    <meta charset="UTF-8">
    <title>Lembar Agenda Surat Keluar - {{ $letter->agenda_number }}</title>
    <style>
        @page {
            size: A4;
            margin: 1.5cm;
...
<body>
    ...
    <div class="no-print">
        <button onclick="window.print()" style="padding: 8px 16px; font-size: 13px; font-weight: bold; cursor: pointer; background: #0f172a; color: white; border: none; border-radius: 6px;">
            Cetak Lembar Agenda Surat Keluar
        </button>
    </div>
    ...
    <div class="title">LEMBAR AGENDA SURAT KELUAR</div>
    ...
    <table class="grid">
        ...
        <tr>
            <td class="label">TANGGAL SURAT KELUAR</td>
            <td>{{ $letter->letter_date->isoFormat('D MMMM Y') }}</td>
        </tr>
        <tr>
            <td class="label">NOMOR SURAT RESMI</td>
            <td><strong>{{ $letter->reference_number }}</strong></td>
        </tr>
        <tr>
            <td class="label">TUJUAN SURAT / PENERIMA</td>
            <td><strong>{{ $letter->destination }}</strong></td>
        </tr>
        <tr>
